...
///
/// :param data_a: The first input signal to FFT convolve. Returned convolution
///     arrays will be "same-length" trimmed to "data_a"'s length.
/// :param data_b: The second input signal to FFT convolve.
/// :return: The FFT convolved result of the same length as input signal
///     "data_a".
#[pyfunction]
...
/// transforming the result back into a signal. This function uses "same-length"
/// triming with the first parameter "data_a". This means that the returned
/// deconvolution's array length will have the same length as "data_a".
///
/// :param data_a: The first input signal to FFT deconvolve. Returned
///     deconvolution arrays will be "same-length" trimmed to "data_a"'s length.
/// :param data_b: The second input signal to deconvolve.
/// :param epsilon: An epsilon value to prevent division by zero errors (default
///     = 1e-8).
/// :return: The FFT deconvolved result of the same length as input signal
///     "data_a".
#[pyfunction]
