...
fn filter_fft_convolve_1d() {
    ...
    let a = decay::ideal_exponential_decay_1d(SAMPLES, PERIOD, &TAUS, &FRACTIONS, TOTAL_COUNTS).unwrap();
...
fn filter_fft_deconvolve_1d() {
    ...
    .unwrap();
    let b = decay::ideal_exponential_decay_1d(SAMPLES, PERIOD, &TAUS, &FRACTIONS, TOTAL_COUNTS).unwrap();
